...
from lib import dataloader_audio as dataloader
...
def main():
    ...
    if args.save_feats:
        ...
        save_features(val_loader, model, criterion, use_audio, args.out_file)
    ...
    for epoch in range(0, epochs):
        ...
        # train one epoch
        train(train_loader, model, criterion, optimizer, epoch, epochs, current_lr, use_audio)

        if nl_nums > 0:
            if args.audio:
                checkpoint_name = 'audio-{}-{}-r-{}-w-{}{}-block.pth.tar'.format(dataset, args.train, arch, nl_nums, nl_type)
            else:
                checkpoint_name = 'noaudio-{}-{}-r-{}-w-{}{}-block.pth.tar'.format(dataset, args.train, arch, nl_nums, nl_type)
        else:
...
def train(train_loader, model, criterion, optimizer, epoch, epochs, current_lr, use_audio):
    ...
    for i, (input, target, audio_feats,_) in enumerate(train_loader):
        ...
        output = model(input, audio_feats, use_audio)
...
def validate(val_loader, model, criterion, use_audio=False):
    ...
    with torch.no_grad():
        ...
        for i, (input, target,audio_feats,img_paths) in enumerate(val_loader):
            target = target.cuda(non_blocking=True)
...
def save_features(val_loader, model, criterion, use_audio, feat_outfile):
    batch_time = AverageMeter()
    # losses = AverageMeter()
    # top1 = AverageMeter()
    # top5 = AverageMeter()
    ...
    with torch.no_grad():
        ...
        for i, (input, target, audio_feats, img_paths) in enumerate(val_loader):
            ...
            # compute output
            feats = model(input, audio_feats, use_audio, get_feats=True)
            # loss = criterion(output, target)
