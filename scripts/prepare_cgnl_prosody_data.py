...
class LfdData():
    def __init__(self, exp, demo_type,skip=1):
        ...
        self.task = exp
        self.img_counts = {}
        t_file = base_dir+'/'+exp+'/train_'+self.task+'_'+self.demo_type+'_prosody.list'
        v_file = base_dir+'/'+exp+'/val_'+self.task+'_'+self.demo_type+'_prosody.list'
        self.f_train = open(t_file,'w')
        self.f_val = open(v_file,'w')
        self.prosody_feat_path = base_dir + 'prosody_feats_A1.pkl'
        with open(self.prosody_feat_path,'rb') as fp:
            self.prosody_feats = pkl.load(fp)
        ...
        for user in self.users:
            ...
            for img_name in img_names:
                ...
                if idx%self.skip==0:
                    ...
                    # TODO: get time for current img frame by using intermediate.mp4
                    ...
                    if flag:
                        flag = False
                        print('zero feats...')
                        # add a noise/none label
                        label = str(extra_label[self.task])
                        # continue

                    # load prosody features
                    
                    
                    demo_id = user+'_'+self.task+'_'+self.demo_type
                    curr_prosody = self.prosody_feats[demo_id] # list of start,stop,features
                    speech_act = False
                    for values in curr_prosody:
                        start, stop = values[0], values[1]
                        # print(start,stop,curr_time)
                        # curr img frame inside a speech act
                        if curr_time>=float(start) and curr_time<=float(stop):
                            features = values[2]
                            feat_names = list(features.keys())
                            feat_names.sort()
                            prosody_feat_values = []
                            for key in features:
                                prosody_feat_values.append(features[key])
                            curr_feats = prosody_feat_values
                            speech_act = True
                            break
                    ...
                    # print(label)
                    if user=='user16' or user=='user18' or user=='user20':
                        # print(user)
                        # exit(1)
